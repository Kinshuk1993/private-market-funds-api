...
└── app/
    ...
    ├── core/
    │   ├── __init__.py
    │   ├── cache.py         # In-memory TTL cache with write-through invalidation
    │   ├── config.py        # Pydantic-settings: env → typed config, SQLite/PG switching
    │   ├── exceptions.py    # Domain exceptions + CircuitBreakerError handler
    │   ├── logging.py       # Production logging: RotatingFileHandler + JSON formatter
    │   └── resilience.py    # Circuit breaker + exponential backoff with jitter
    ├── db/
    │   ├── __init__.py
...
| <http://localhost:8000/health> | Liveness / readiness probe with DB, circuit breaker, and cache stats |
...
| `CB_FAILURE_THRESHOLD` | `5` | Consecutive failures before circuit breaker opens |
| `CB_RECOVERY_TIMEOUT` | `30.0` | Seconds to wait before circuit breaker probe |
